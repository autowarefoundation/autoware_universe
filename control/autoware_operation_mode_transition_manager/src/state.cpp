...
bool AutonomousMode::isModeChangeCompleted()
{
  ...
  if (trajectory_.points.size() < 2) {
    RCLCPP_INFO_THROTTLE(
      logger_, *get_clock(), 3000, "Not stable yet: trajectory size must be > 2");
    return unstable();
  }
  ...
  const auto closest_idx = findNearestIndex(
    ...
    nearest_yaw_deviation_threshold_);
  if (!closest_idx) {
    RCLCPP_INFO_THROTTLE(logger_, *get_clock(), 3000, "Not stable yet: closest point not found");
    return unstable();
  }
  ...
  if (std::isnan(dist_deviation)) {
    RCLCPP_INFO_THROTTLE(
      logger_, *get_clock(), 3000, "Not stable yet: lateral offset calculation failed.");
    return unstable();
  }
  if (dist_deviation > stable_check_param_.dist_threshold) {
    RCLCPP_INFO_THROTTLE(
      logger_, *get_clock(), 3000, "Not stable yet: distance deviation is too large: %f",
      dist_deviation);
    return unstable();
  }
  ...
  if (yaw_deviation > stable_check_param_.yaw_threshold) {
    RCLCPP_INFO_THROTTLE(
      logger_, *get_clock(), 3000, "Not stable yet: yaw deviation is too large: %f", yaw_deviation);
    return unstable();
  }
  ...
  if (speed_deviation > stable_check_param_.speed_upper_threshold) {
    RCLCPP_INFO_THROTTLE(
      logger_, *get_clock(), 3000, "Not stable yet: ego speed is too high: %f", speed_deviation);
    return unstable();
  }
  if (speed_deviation < stable_check_param_.speed_lower_threshold) {
    RCLCPP_INFO_THROTTLE(
      logger_, *get_clock(), 3000, "Not stable yet: ego speed is too low: %f", speed_deviation);
    return unstable();
  }
  ...
  RCLCPP_INFO_THROTTLE(logger_, *get_clock(), 3000, "Now stable: now duration: %f", stable_time);
...
bool AutonomousMode::isModeChangeAvailable()
{
  ...
  if (!closest_idx) {
    RCLCPP_INFO_THROTTLE(
      logger_, *get_clock(), 3000, "Engage unavailable: closest point not found");
    debug_info_ = DebugInfo{};  // all false
