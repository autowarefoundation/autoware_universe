    ...
    sub_transform_ = this->create_subscription<tf2_msgs::msg::TFMessage>(
      node_param_.topic, qos, [this](tf2_msgs::msg::TFMessage::ConstSharedPtr msg) {
        for (const auto & transform : msg->transforms) {
          ...
          if (node_param_.enable_keep_publishing) {
            last_tf_topic_ = msg;
          } else if (is_throttle_period_elapsed()) {
...
void TopicRelayController::on_topic(std::shared_ptr<const rclcpp::SerializedMessage> msg)
{
  if (!is_relaying_) return;

  if (node_param_.enable_keep_publishing) {
    last_topic_ = std::move(msg);
    return;
  }

  if (!is_throttle_period_elapsed()) return;
