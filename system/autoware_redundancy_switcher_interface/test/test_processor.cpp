...
protected:
  ...
  void set_stable()
  {
    processor_.handle(InputEvent{
      SetSwitcherSignalsEvent{Annotated<SwitcherSignals>{SwitcherSignals{true, false, false}, "stable"}}});
  }
  void set_self_interrupted()
  {
    processor_.handle(InputEvent{
      SetSwitcherSignalsEvent{Annotated<SwitcherSignals>{SwitcherSignals{false, true, false}, "self_interrupted"}}});
  }
  void set_faulted()
  {
    processor_.handle(InputEvent{
      SetSwitcherSignalsEvent{Annotated<SwitcherSignals>{SwitcherSignals{false, false, true}, "faulted"}}});
  }};
...
TEST_F(ProcessorTest, SetAutowareReady_TriggersDiagOnChange)
{
  const auto commands = processor_.handle(InputEvent{
    SetAutowareReadyEvent{Annotated<AutowareReady>{AutowareReady::True, "test"}}});
...
{
  processor_.handle(InputEvent{SetVelocityStatusEvent{Annotated<VelocityStatus>{VelocityStatus::Stopped, "test"}}});
  const auto commands = processor_.handle(InputEvent{
    SetVelocityStatusEvent{Annotated<VelocityStatus>{VelocityStatus::Stopped, "test"}}});
  EXPECT_FALSE(has_effect<UpdateStatusDiagCommand>(commands));
}
...
{
  const auto commands = processor_.handle(InputEvent{
    SetControlModeEvent{Annotated<ControlMode>{ControlMode::Auto, "test"}}});
  EXPECT_TRUE(has_effect<UpdateStatusDiagCommand>(commands));
}

TEST_F(ProcessorTest, SetControlMode_DoesNotTriggerDiagIfNoChange)
{
  processor_.handle(InputEvent{SetControlModeEvent{Annotated<ControlMode>{ControlMode::Auto, "test"}}});
  const auto commands = processor_.handle(InputEvent{
    SetControlModeEvent{Annotated<ControlMode>{ControlMode::Auto, "test"}}});
  EXPECT_FALSE(has_effect<UpdateStatusDiagCommand>(commands));
}
...
TEST_F(ProcessorTest, SetSwitcherSignals_Stable_TriggersDiagAndLog)
{
  const auto commands = processor_.handle(
    InputEvent{SetSwitcherSignalsEvent{Annotated<SwitcherSignals>{{true, false, false}, "stable"}}});
...
TEST_F(ProcessorTest, SetSwitcherSignals_SelfInterrupted_EmitsEmptyActiveControlUnit)
{
  ...
  const auto commands = processor_.handle(InputEvent{
    SetSwitcherSignalsEvent{Annotated<SwitcherSignals>{{false, true, false}, "self_interrupted"}}});
...
TEST_F(ProcessorTest, SetActiveControlUnit_PassthroughWhenStable)
{
  ...
  const auto commands = processor_.handle(InputEvent{
    SetActiveControlUnitEvent{Annotated<ActiveControlUnit>{expected, "test"}}});
  const auto * cmd = find_effect<UpdateActiveControlUnitCommand>(commands);
  ASSERT_NE(cmd, nullptr);
...
TEST_F(ProcessorTest, SetActiveControlUnit_ForcedEmptyWhenSelfInterrupted)
{
  ...
  const auto commands = processor_.handle(InputEvent{
    SetActiveControlUnitEvent{Annotated<ActiveControlUnit>{non_empty, "test"}}});
  const auto * cmd = find_effect<UpdateActiveControlUnitCommand>(commands);
  ASSERT_NE(cmd, nullptr);
  EXPECT_TRUE(cmd->value.unit_ids.empty());
}
