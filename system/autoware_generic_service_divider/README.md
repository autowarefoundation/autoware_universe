...
| Divider plugins             | `plugins/*.cpp`                        | Declare _what_ to divide: the service type, the input/output configuration, success/error judgment, and the type-specific `copy_request()`/`copy_response()`. |

The type-erased service server/client themselves (`GenericService`/`GenericClient`) are not part of this package: they come from `autoware_agnocast_wrapper`, which the node reaches through the `AUTOWARE_CREATE_GENERIC_SERVICE*`/`AUTOWARE_CREATE_GENERIC_CLIENT*` macros and `AUTOWARE_GENERIC_SERVICE_PTR`/`AUTOWARE_GENERIC_CLIENT_PTR` types (see `include/autoware/agnocast_wrapper/generic_service.hpp`/`generic_client.hpp` in that package). This is also what gives the divider its runtime rclcpp/Agnocast switch (see "Agnocast" below): the plugin code itself never branches on which backend is active.
...
This node's behavior is identical regardless of `ENABLE_AGNOCAST`; only the transport underneath the generic service/client changes. `autoware_agnocast_wrapper_register_node()` (in `CMakeLists.txt`) generates a `main()` that, at runtime, spins the node on `MultiThreadedExecutor` when Agnocast is disabled and on `agnocast::MultiThreadedAgnocastExecutor` when it is enabled (`use_agnocast()` returning true); the launch file includes `autoware_agnocast_wrapper`'s `agnocast_env.launch.xml` to set up the matching `LD_PRELOAD`. Each output client and the input service get their own dedicated callback group (see "Request forwarding" below) because `MultiThreadedAgnocastExecutor` refuses to mix an Agnocast-backed callback and a plain rclcpp callback in the same `MutuallyExclusive` group.

Every request/response the divider itself allocates (as opposed to one received from a peer) is obtained from the owning `GenericClient`'s/`GenericService`'s own `create_request()`/`create_response()` and filled via the plugin's `copy_request()`/`copy_response()`, rather than being handed to a _different_ client/service's `async_send_request()`/`send_response()` directly. This is required on the Agnocast backend, where a borrowed shared-memory buffer belongs to the specific endpoint that lent it (see `forward_request()`'s and `build_response()`'s doc comments in `service_divider_plugin_base.cpp`, and `autoware_agnocast_wrapper`'s own doc comments on this requirement) -- and is followed on the ROS2 backend too, even though it is not load-bearing there, so the two backends run the exact same code path.

### Startup gating (fail-closed)

`setup_service_division()` creates one `AUTOWARE_GENERIC_CLIENT_PTR` per output service and then calls `try_start_input_service()`.
...
Each output client is placed in its own `MutuallyExclusive` callback group, and the input service is placed in a `Reentrant` callback group, so concurrent input calls are tracked independently. The node executable runs on `MultiThreadedExecutor` or `agnocast::MultiThreadedAgnocastExecutor` depending on `ENABLE_AGNOCAST` (see "Agnocast" above).
...
11. **The generic service/client come from `autoware_agnocast_wrapper`, not this package.** Jazzy's `rclcpp::GenericClient` has no callback-based `async_send_request()` overload and no server-side counterpart, and Humble has neither at all, so that package hand-rolls both (`ROS2GenericService`/`ROS2GenericClient`) as the ROS2-backend fallback for every supported distro, alongside the Agnocast-backed versions. See its own doc comments for the rationale and for the (intentionally minor) behavioral differences that remain between the two backends.
...
13. **A plugin with no configured outputs answers every call with an error.** A plugin listed in `plugins` whose configuration block is missing or misnamed falls back to its hardcoded default `input_service` with an empty output list. The startup gate sees nothing to wait for, so the input service **is** advertised, and `handle_request()` answers immediately with `"Primary service did not respond"` (there being no output to designate as primary) instead of leaving the caller waiting. When adding a plugin to `plugins`, check that its parameter prefix matches the block name in the configuration file, and that `output_services.names` is non-empty -- the node will run without complaint, but every call to it will fail.
...
- **A timed-out output call is never actually abandoned client-side, and so grows without bound.** `forward_request()`'s timeout only stops the divider itself from _waiting_ (known limit 8); it never tells `GenericClient` to give up on the in-flight call. `GenericClient` has no API for that (`cancel_request()` only covers a request that was created but never sent -- see its doc comment in `autoware_agnocast_wrapper`), so the response callback passed to `async_send_request()`, which captures the whole `PendingDivision` by value, stays registered inside `GenericClient` forever waiting for a response that will never come. This is exactly the redundant-ECU-down scenario this node exists to handle: an output that is currently down and being retried leaks one such registration -- and the `PendingDivision` it keeps alive -- per call, for the lifetime of the node. `rclcpp::Client` exposes `remove_pending_request()`/`prune_requests_older_than()` for this purpose upstream; `GenericClient` would need an equivalent, called from the timeout path.
...
| `SubServerTimeout`                                     | A non-primary output server disappearing after advertisement, so `"One or more output services failed or timed out"` is reached via a timeout rather than a failure response           |
| `PrimaryServerTimeout`                                 | The **primary** output server disappearing, so the separate `"Primary service did not respond"` response path is reached (not exercised by any failure/non-primary-timeout test above) |
| `NoOutputServicesRespondsInsteadOfHanging`             | A plugin configured with zero output services still answers every call with an error instead of leaving it pending forever (see known limit 13)                                        |
...
`SubServerTimeout` and `PrimaryServerTimeout` start both output servers first and remove one only after the input service has been advertised. Removing that setup makes the test unreachable, because the input service would never be advertised in the first place.

Every test in this file runs against **both** backends: the fixture's `SetUp()` selects `agnocast::MultiThreadedAgnocastExecutor` instead of `rclcpp::executors::MultiThreadedExecutor` when `use_agnocast()` is true, so a colcon build with `ENABLE_AGNOCAST=1` and the Agnocast heaphook preloaded (`LD_PRELOAD`) exercises the exact same scenarios against the real Agnocast-backed generic service/client, not just the ROS2 fallback (the fixture skips, rather than crashing, if `ENABLE_AGNOCAST=1` but the heaphook is not preloaded -- Agnocast otherwise exits the process from inside the endpoint constructor in that case).
