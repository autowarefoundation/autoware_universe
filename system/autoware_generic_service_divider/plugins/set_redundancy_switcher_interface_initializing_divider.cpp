...
class SetRedundancySwitcherInterfaceInitializingDivider : public ServiceDividerPluginBase
{
public:
  void initialize(rclcpp::Node::SharedPtr node) override
  {
    ...
    input_name_ = node_->declare_parameter<std::string>(
      "set_redundancy_switcher_interface_initializing.input_service",
      "/system/redundancy_switcher_interface/set_initializing");

    const auto output_names = node_->declare_parameter<std::vector<std::string>>(
      "set_redundancy_switcher_interface_initializing.output_services.names",
      std::vector<std::string>{});
    const auto output_primaries = node_->declare_parameter<std::vector<bool>>(
      "set_redundancy_switcher_interface_initializing.output_services.primaries",
      std::vector<bool>{});
    const auto output_timeouts = node_->declare_parameter<std::vector<int64_t>>(
      "set_redundancy_switcher_interface_initializing.output_services.timeouts_ms",
      std::vector<int64_t>{});
...
PLUGINLIB_EXPORT_CLASS(
  generic_service_divider::SetRedundancySwitcherInterfaceInitializingDivider,
