...
TEST_F(CudaCropBoxFilterTest, AcceptsAnUnusualFieldOrder)
{
  CudaCropBoxFilter filter(box(false));

  // The kernel addresses every coordinate as offset + index * point_step and
  // never assumes an order, so a cloud whose field descriptors are listed z, y,
  // x is as readable as one listed x, y, z. Refusing it -- which comparing
  // against a named point type would -- would reject a cloud this can process.
  auto reordered = makeCloud({point(1.0f, 2.0f, 3.0f)});
  std::swap(reordered.fields[0], reordered.fields[2]);
  const auto output = filter.filter(reordered);
  ASSERT_NE(output, nullptr);
