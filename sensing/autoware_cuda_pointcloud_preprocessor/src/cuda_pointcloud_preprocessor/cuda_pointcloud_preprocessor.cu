  ...
  const T * input, std::uint32_t * output, std::size_t count, T value, cudaStream_t stream)
{
  ...
  cub::TransformInputIterator<std::uint32_t, EqualsValue<T>, const T *> transform_iterator(
    input, EqualsValue<T>{value});
...
void CudaPointcloudPreprocessor::organizePointcloud()
{
  ...
  cub::DeviceSegmentedRadixSort::SortKeys(
    reinterpret_cast<void *>(thrust::raw_pointer_cast(device_scratch_workspace_.data())),  // NOLINT
