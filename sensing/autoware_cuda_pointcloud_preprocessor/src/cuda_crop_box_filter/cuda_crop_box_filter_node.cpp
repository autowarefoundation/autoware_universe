...
void CudaCropBoxFilterNode::warnAboutLayoutOnce()
{
  ...
  // Name the requirement that failed rather than the layout that did not match:
  // this filter has no layout, only requirements, and "x is FLOAT64" and "there
  // is no x" need different fixes upstream.
  RCLCPP_ERROR(
    get_logger(), "Cannot read this cloud: %s. Dropping.", filter_->lastLayoutError().c_str());
