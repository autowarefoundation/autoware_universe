...
protected:
    ...
    const std::shared_ptr<T> & distortion_corrector, const rclcpp::Time & timestamp)
  {
    ...
    for (const auto & twist_msg : twist_msgs) {
      distortion_corrector->insert_twist(twist_msg);
    }
  }
    ...
    const std::shared_ptr<T> & distortion_corrector, const rclcpp::Time & timestamp)
  {
    ...
    for (const auto & imu_msg : imu_msgs) {
      distortion_corrector->insert_imu(imu_msg);
    }
  }
...
TEST_F(DistortionCorrectorTest, TestInsertTwist)
{
  rclcpp::Time timestamp(timestamp_seconds, timestamp_nanoseconds, RCL_ROS_TIME);
  auto twist_msg = generate_twist_msg(twist_linear_x, twist_angular_z, timestamp);
  distortion_corrector_2d_->insert_twist(twist_msg);
...
TEST_F(DistortionCorrectorTest, TestInsertImu)
{
  rclcpp::Time timestamp(timestamp_seconds, timestamp_nanoseconds, RCL_ROS_TIME);
  ...
  distortion_corrector_2d_->insert_imu(imu_msg);
...
TEST_F(DistortionCorrectorTest, TestUndistortPointcloudPrunesOldSensorSamples)
{
  const rclcpp::Time timestamp(timestamp_seconds, timestamp_nanoseconds, RCL_ROS_TIME);
  const auto old_stamp = subtract_milliseconds(timestamp, 200);
  const auto latest_before_first_point_stamp = subtract_milliseconds(timestamp, 50);
  const auto after_first_point_stamp = add_milliseconds(timestamp, 20);

  distortion_corrector_2d_->insert_twist(
    generate_twist_msg(twist_linear_x, twist_angular_z, old_stamp));
  distortion_corrector_2d_->insert_twist(
    generate_twist_msg(twist_linear_x, twist_angular_z, latest_before_first_point_stamp));
  distortion_corrector_2d_->insert_twist(
    generate_twist_msg(twist_linear_x, twist_angular_z, after_first_point_stamp));

  distortion_corrector_2d_->set_imu_transform(get_imu_to_base_link_transform());
  distortion_corrector_2d_->insert_imu(
    generate_imu_msg(imu_angular_x, imu_angular_y, imu_angular_z, old_stamp));
  distortion_corrector_2d_->insert_imu(
    generate_imu_msg(imu_angular_x, imu_angular_y, imu_angular_z, latest_before_first_point_stamp));
  distortion_corrector_2d_->insert_imu(
    generate_imu_msg(imu_angular_x, imu_angular_y, imu_angular_z, after_first_point_stamp));
  ...
  distortion_corrector_2d_->undistort_pointcloud(
    true, std::nullopt, get_base_link_transform(), pointcloud);

  ASSERT_EQ(distortion_corrector_2d_->get_twist_queue().size(), 2U);
  EXPECT_EQ(
    rclcpp::Time(distortion_corrector_2d_->get_twist_queue().front().header.stamp).nanoseconds(),
    latest_before_first_point_stamp.nanoseconds());
  ASSERT_EQ(distortion_corrector_2d_->get_angular_velocity_queue().size(), 2U);
  EXPECT_EQ(
    rclcpp::Time(distortion_corrector_2d_->get_angular_velocity_queue().front().header.stamp)
      .nanoseconds(),
    latest_before_first_point_stamp.nanoseconds());
}
...
TEST_F(DistortionCorrectorTest, TestUndistortPointcloudWithBaseLinkTransform)
{
  const rclcpp::Time timestamp(timestamp_seconds, timestamp_nanoseconds, RCL_ROS_TIME);
  auto twist_msg = generate_twist_msg(twist_linear_x, twist_angular_z, timestamp);
  distortion_corrector_2d_->insert_twist(twist_msg);
  ...
  auto pointcloud = generate_pointcloud_msg(false, timestamp, default_points, default_azimuths);
  distortion_corrector_2d_->undistort_pointcloud(
    false, std::nullopt, get_base_link_transform(), pointcloud);
...
TEST_F(DistortionCorrectorTest, TestUndistortPointcloudWithLidarFrameTransform)
{
  const rclcpp::Time timestamp(timestamp_seconds, timestamp_nanoseconds, RCL_ROS_TIME);
  auto twist_msg = generate_twist_msg(twist_linear_x, twist_angular_z, timestamp);
  distortion_corrector_2d_->insert_twist(twist_msg);
  ...
  distortion_corrector_2d_->undistort_pointcloud(
    false, std::nullopt, get_lidar_to_base_link_transform(), pointcloud);
...
TEST_F(DistortionCorrectorTest, TestUndistortPointcloudWithEmptyPointCloud)
{
  ...
  distortion_corrector_2d_->undistort_pointcloud(
    true, std::nullopt, get_base_link_transform(), empty_pointcloud);
...
TEST_F(DistortionCorrectorTest, TestUndistortPointcloud2dWithImuInLidarFrame)
{
  ...
  generate_and_process_imu_msgs(distortion_corrector_2d_, timestamp);
  distortion_corrector_2d_->undistort_pointcloud(
    true, std::nullopt, get_lidar_to_base_link_transform(), pointcloud);
...
TEST_F(DistortionCorrectorTest, TestUndistortPointcloud3dWithoutImuInBaseLink)
{
  ...
  // Test using only twist
  distortion_corrector_3d_->undistort_pointcloud(
    false, std::nullopt, get_base_link_transform(), pointcloud);
...
TEST_F(DistortionCorrectorTest, TestUndistortPointcloudWithPureLinearMotion)
{
  ...
  distortion_corrector_2d_->insert_twist(twist_msg);
  distortion_corrector_2d_->undistort_pointcloud(
    false, std::nullopt, get_base_link_transform(), test2d_pointcloud);

  distortion_corrector_3d_->insert_twist(twist_msg);
  distortion_corrector_3d_->undistort_pointcloud(
    false, std::nullopt, get_base_link_transform(), test3d_pointcloud);
...
TEST_F(DistortionCorrectorTest, TestUndistortPointcloudNotUpdatingAzimuthAndDistance)
{
  ...
  EXPECT_THROW(
    {
      distortion_corrector_2d_->undistort_pointcloud(
        true, angle_conversion_opt, get_base_link_transform(), pointcloud_base_link);
    },
    std::runtime_error);
  ...
  distortion_corrector_2d_->undistort_pointcloud(
    true, angle_conversion_opt, get_lidar_to_base_link_transform(), pointcloud_lidar_top);
...
TEST_F(DistortionCorrectorTest, TestUndistortPointcloudUpdateAzimuthAndDistanceInVelodyne)
{
  ...
  distortion_corrector_2d_->undistort_pointcloud(
    true, angle_conversion_opt, get_lidar_to_base_link_transform(), pointcloud);
