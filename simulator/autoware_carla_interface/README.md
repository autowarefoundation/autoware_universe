...
| `wait_for_initialpose`            | bool   | False                                                                             | Spawn the ego on the first pose received on `/initialpose` rather than at `spawn_point`, so the ego is never placed somewhere else first. Default False keeps the original order: spawn at `spawn_point`, then move the ego when a pose arrives.                                                                                                                                    |
| `initialpose_wait_timeout`        | double | 60.0                                                                              | Seconds to wait for that pose before failing the startup (only used when `wait_for_initialpose` is True).                                                                                                                                                                                                                                                                           |
