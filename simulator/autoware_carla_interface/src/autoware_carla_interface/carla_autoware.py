...
class InitializeInterface(object):
    ...
    def _load_carla_world(self, client):
        ...
        if self.force_load_world:
            self._load_world(client)
            return

        if hasattr(client, "load_world_if_different") and self._try_load_world_if_different(client):
            return

        if self._current_world_name(client) != self.carla_map:
            self._load_world(client)

    def _load_world(self, client) -> None:
        """Load the map via ``client.load_world``; tolerate non-connection failures."""
    ...
    def _try_load_world_if_different(self, client) -> bool:
        """Try the newer ``load_world_if_different`` API; return True on success."""
        try:
            print(
    ...
    @staticmethod
    def _current_world_name(client):
        """Return the loaded world's short map name, or None if unavailable."""
        try:
