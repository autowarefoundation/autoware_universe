...
bool TrtCommon::setup(ProfileDimsPtr profile_dims, NetworkIOPtr network_io)
{
  ...
  // validateNetworkIO.
  if (network_io_ && trt_config_->strongly_typed) {
    logger_->log(
...
bool TrtCommon::initialize()
{
  ...
  // Precision flags and constraints do not apply to strongly typed networks.
  if (trt_config_->strongly_typed) {
    if (trt_config_->precision == "fp16" || trt_config_->precision == "int8") {
      logger_->log(
        nvinfer1::ILogger::Severity::kINFO,
        "Ignoring precision setting '%s': the network is strongly typed",
        trt_config_->precision.c_str());
    }
  } else if (trt_config_->precision == "fp16") {
...
{
  return trt_config_->strongly_typed;
}
