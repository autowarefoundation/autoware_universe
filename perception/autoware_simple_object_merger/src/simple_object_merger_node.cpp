...
: Node("simple_object_merger", node_options)
{
  ...
  if (input_topic_size == 2) {
    ...
    input0_.subscribe(this, node_param_.topic_names.at(0), rclcpp::QoS{1}.best_effort().get_rmw_qos_profile());
    input1_.subscribe(this, node_param_.topic_names.at(1), rclcpp::QoS{1}.best_effort().get_rmw_qos_profile());
    ...
    for (size_t i = 0; i < input_topic_size; i++) {
      std::function<void(const DetectedObjects::ConstSharedPtr msg)> func =
        ...
        create_subscription<DetectedObjects>(node_param_.topic_names.at(i), rclcpp::QoS{1}.best_effort(), func);
    }
