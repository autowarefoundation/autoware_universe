...
In the case where the target object is inside the road, the additional path(s) to reach the nearest crosswalk are only generated if the distance between the object and the crosswalk is not higher than parameter `min_crosswalk_user_on_road_distance`.
...
| `min_crosswalk_user_on_road_distance`                            | [m/s] | double | crosswalk users on road or shoulder lanelets are only predicted to cross the crosswalk if they are within this distance               |
