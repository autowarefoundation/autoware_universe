...
**This tool has no config file.** The implementation is also a single file, `traffic_light_pipeline_bag2bag_runner.cpp`.

- **All tuning values of the first stage** are read from this package's `config/traffic_light_recognition.param.yaml`. This is the same file the launch file gives to the Node, so each threshold is defined in one place only, and by design this tool cannot measure values different from production. (`map_based_detector.min/max_timestamp_offset` works the same way: `CameraConfig` holds a value per camera, but every camera gets the value from the package config.) There is no way to override them.
