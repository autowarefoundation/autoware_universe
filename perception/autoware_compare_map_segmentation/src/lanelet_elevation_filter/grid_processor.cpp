...
{
  // First pass: calculate statistics and clear points
  for (auto & [index, cell] : grid_cells_) {
    ...
    // Clear points vector - but don't shrink yet
    cell.points.clear();
  }
...
std::optional<double> GridProcessor::getArtificalElevationAtPoint(double x, double y) const
{
  GridIndex index = getGridIndex(x, y);
  ...
  const GridIndex & index) const
{
  ...
  // No neighbors found - return std::nullopt to indicate no valid data
  return std::nullopt;
}
  ...
  double x, double y, double z, double threshold, bool require_map_coverage) const
{
  ...
  if (has_map_data) {
    ...
    auto elevation_opt = getArtificalElevationAtPoint(x, y);
    if (!elevation_opt.has_value()) {
      // No valid elevation data available - reject the point
      return false;
    }
  ...
  const std::string & cache_directory)
{
  ...
  // Fill empty grids around original points with specified extension
