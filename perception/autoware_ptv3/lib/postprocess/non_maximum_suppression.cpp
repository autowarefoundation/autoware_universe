...
{
  if (params.search_distance_2d_ <= 0.0) {
    throw std::runtime_error("IoU NMS search distance must be positive.");
  }
  if (params.iou_threshold_ < 0.0 || params.iou_threshold_ > 1.0) {
    throw std::runtime_error("IoU NMS threshold must be between 0 and 1.");
  }
  params_ = params;
