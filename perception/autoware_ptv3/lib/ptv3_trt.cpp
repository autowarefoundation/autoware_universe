...
void PTv3TRT::initEncoderTrt(const tensorrt_common::TrtCommonConfig & trt_config)
{
  ...
  // The encoder consumes the level-0 serialization order directly; it used to take the raw codes
  // and argsort them in-graph, duplicating work the preprocessing already does for the pooling
  // metadata. serialized_code stays a host-side buffer for chaining the pooling stages.
  ...
  encoder_trt_ptr_->setTensorAddress("serialized_order", pre_ptr_->level0SerializedOrder());
  encoder_trt_ptr_->setTensorAddress("serialized_inverse", pre_ptr_->level0SerializedInverse());
...
void PTv3TRT::initSeg3dHeadTrt(const tensorrt_common::TrtCommonConfig & trt_config)
{
  ...
  for (std::size_t stage = 0; stage + 1 < stage_count; ++stage) {
    ...
    if (stage == 0) {
      seg3d_head_trt_ptr_->setTensorAddress("serialized_order", pre_ptr_->level0SerializedOrder());
      seg3d_head_trt_ptr_->setTensorAddress(
        "serialized_inverse", pre_ptr_->level0SerializedInverse());
