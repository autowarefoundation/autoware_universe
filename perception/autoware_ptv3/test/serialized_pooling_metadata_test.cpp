...
TEST_F(SerializedPoolingMetadataTest, MatchesCpuReferenceForOnnxFacingInputs)
{
  ...
  // The level-0 order and inverse are published for the encoder engine to consume directly. Row 0
  // must be the identity (the caller guarantees voxels arrive in order-0 code order) and every
  // other row must be a genuine argsort of that row's codes.
  {
    const auto count = static_cast<std::size_t>(num_voxels);
    const auto level0_order = copyToHost(preprocess.level0SerializedOrder(), count * kNumOrders);
    const auto level0_inverse =
      copyToHost(preprocess.level0SerializedInverse(), count * kNumOrders);
    for (std::size_t order = 0; order < kNumOrders; ++order) {
      ...
      expect_equal(
        std::vector<std::int64_t>(level0_order.begin() + begin, level0_order.begin() + end),
        expected_order, "level0 serialized_order" + suffix);
      expect_equal(
        std::vector<std::int64_t>(level0_inverse.begin() + begin, level0_inverse.begin() + end),
        expected_inverse, "level0 serialized_inverse" + suffix);
    }
    expect_orders_diverge(level0_order, count, kNumOrders, "level0 serialized_order");
  }
