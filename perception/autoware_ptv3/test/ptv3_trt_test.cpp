...
// The shared fixture's default segmentation class names are not in the PTv3 taxonomy. That goes
// unnoticed in tests that only build a PTv3Config, but PTv3TRT constructs the postprocess, which
// maps every class name and throws on an unknown one.
...
TEST_F(PTv3TRTTest, StageProfileCountsAnchorTheFinestStageToTheConfiguredMinimum)
{
  ...
  const auto counts = trt.stageProfileCounts(0);
...
TEST_F(PTv3TRTTest, StageProfileCountsAllowPooledStagesToCollapseToOneVoxel)
{
  ...
  for (std::size_t stage = 1; stage < params.enc_channels.size(); ++stage) {
    EXPECT_EQ(trt.stageProfileCounts(stage)[0], 1) << "stage " << stage;
  }
}
...
TEST_F(PTv3TRTTest, StageProfileCountsStayOrderedAndWithinTheGeometricBound)
{
  ...
  for (std::size_t stage = 0; stage < params.enc_channels.size(); ++stage) {
    const auto counts = trt.stageProfileCounts(stage);
    EXPECT_LE(counts[0], counts[1]) << "stage " << stage;
    EXPECT_LE(counts[1], counts[2]) << "stage " << stage;
    EXPECT_EQ(counts[2], config.stage_voxel_capacity(stage)) << "stage " << stage;
  }
}
...
TEST_F(PTv3TRTTest, StageProfileCountsShrinkTheOptimumWithPoolingDepth)
{
  ...
  EXPECT_GE(trt.stageProfileCounts(0)[1], trt.stageProfileCounts(1)[1]);
  EXPECT_GE(trt.stageProfileCounts(1)[1], trt.stageProfileCounts(2)[1]);
}
...
TEST_F(PTv3TRTTest, StageProfileCountsClampTheOptimumIntoTheValidRange)
{
  ...
  for (std::size_t stage = 0; stage < params.enc_channels.size(); ++stage) {
    const auto counts = trt.stageProfileCounts(stage);
    EXPECT_GE(counts[1], counts[0]) << "stage " << stage;
    EXPECT_LE(counts[1], config.stage_voxel_capacity(stage)) << "stage " << stage;
  }
}
...
{
  // The layout predicates only require a prefix match, so the format is pinned by field count.
  // A cloud with XYZI at the front plus a trailing field is not a layout this node can consume.
  const TestablePTv3TRT trt{makeConfig(baseParams())};
