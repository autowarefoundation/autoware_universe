...
CameraDataStore::CameraDataStore(rclcpp::Node * node, const int rois_number, const int image_height, const int image_width, const int anchor_camera_id, const bool is_compressed_image, const int decompression_downsample)
...
{
  if (decompression_downsample_ !=1 && decompression_downsample_ != 2 && decompression_downsample_ != 4 && decompression_downsample_ != 8) {
    throw std::runtime_error("Invalid decompression downsample: " + std::to_string(decompression_downsample_) + " . Should be 2, 4, or 8.");
  }

  image_input_ = std::make_shared<Tensor>("image_input", nvinfer1::Dims{5,1,rois_number,3,image_height,image_width}, nvinfer1::DataType::kFLOAT); // {num_dims, batch_size, rois_number, num_channels, height, width}

  image_input_mean_ = std::make_shared<Tensor>("image_input_mean", nvinfer1::Dims{1,3}, nvinfer1::DataType::kFLOAT);
  image_input_mean_->load_from_vector({103.530, 116.280, 123.675});
  image_input_std_ = std::make_shared<Tensor>("image_input_std", nvinfer1::Dims{1,3}, nvinfer1::DataType::kFLOAT);
...
void CameraDataStore::update_camera_image_compressed(const int camera_id, const CompressedImage::ConstSharedPtr & input_compressed_image_msg)
{
  ...
  if (H < image_height_ || W < image_width_) {
    throw std::runtime_error("Decoded image size is smaller than the model input size: H: " + std::to_string(H) + ", W: " + std::to_string(W));
  }

  preprocess_image(camera_id, cv_ptr->image.data, H, W);


  camera_info_timestamp_[camera_id] = input_compressed_image_msg->header.stamp.sec + input_compressed_image_msg->header.stamp.nanosec * 1e-9;
...
{
  auto start_time = std::chrono::high_resolution_clock::now();
  preprocess_image(camera_id, input_camera_image_msg->data.data(), input_camera_image_msg->height, input_camera_image_msg->width);
  
  camera_info_timestamp_[camera_id] = input_camera_image_msg->header.stamp.sec + input_camera_image_msg->header.stamp.nanosec * 1e-9;
...
void CameraDataStore::preprocess_image(const int camera_id, const std::uint8_t * input_image_data, const int original_height, const int original_width){
  ...
  float bottom_crop_portion = 0.0f;  // what portion of bottom to crop. We only crop at the top along height, so hardcoded to 0.0f
  ...
  std::unique_ptr<Tensor> image_input_tensor = std::make_unique<Tensor>("camera_img", nvinfer1::Dims{3, original_height,original_width,3}, nvinfer1::DataType::kUINT8);
  cudaMemcpyAsync(
        image_input_tensor->ptr, input_image_data,
        image_input_tensor->nbytes(), cudaMemcpyHostToDevice, stream_);
  ...
  auto err = resizeAndExtractRoi_launch(
    ...
    static_cast<const float*>(image_input_std_->ptr),
    stream_
  );
  if (err != cudaSuccess) {
...
std::vector<float> CameraDataStore::get_camera_info_vector() const
{
  ...
  for (size_t camera_id = 0; camera_id < camera_info_list_.size(); ++camera_id)
  {
    ...
    std::vector<float> K_4x4 = {
      static_cast<float>(camera_info_msg->p[0]), static_cast<float>(camera_info_msg->p[1]), static_cast<float>(camera_info_msg->p[2]), static_cast<float>(camera_info_msg->p[3]),
      static_cast<float>(camera_info_msg->p[4]), static_cast<float>(camera_info_msg->p[5]), static_cast<float>(camera_info_msg->p[6]), static_cast<float>(camera_info_msg->p[7]),
      static_cast<float>(camera_info_msg->p[8]), static_cast<float>(camera_info_msg->p[9]), static_cast<float>(camera_info_msg->p[10]), static_cast<float>(camera_info_msg->p[11]),
      0.f, 0.f, 0.f, 1.f
    };
...
void CameraDataStore::save_processed_image(const int camera_id, const std::string& filename) const
{
  ...
  cudaError_t err = cudaMemcpyAsync(
    cpu_image_data.data(),
    static_cast<const float*>(image_input_->ptr) + camera_offset,
  ...
  for (int h = 0; h < image_height_; ++h) {
    for (int w = 0; w < image_width_; ++w) {
      for (int c = 0; c < 3; ++c) {
        processed_image.at<cv::Vec3f>(h, w)[c] = cpu_image_data[c * image_height_ * image_width_ + h * image_width_ + w]*image_input_std[c] + image_input_mean[c];
      }
    }
  }
  ...
  if (success) {
    ...
    RCLCPP_ERROR(logger_, "Failed to save processed image for camera %d to: %s", camera_id, filename.c_str());
  }
}
