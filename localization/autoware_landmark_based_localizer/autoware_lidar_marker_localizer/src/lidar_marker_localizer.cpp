...
: Node("lidar_marker_localizer", node_options), is_activated_(false)
{
  ...
  param_.queue_size_for_output_pose =
    this->declare_parameter<uint64_t>("queue_size_for_output_pose");
  ...
  param_.reference_ring_number = this->declare_parameter<uint8_t>("reference_ring_number");
  ...
  param_.queue_size_for_debug_pub_msg =
    this->declare_parameter<uint64_t>("queue_size_for_debug_pub_msg");
  ...
  const PointCloud2::ConstSharedPtr & points_msg_ptr)
{
  ...
  pcl::PointCloud<PointT>::Ptr points_ptr(new pcl::PointCloud<PointT>);
  ...
  const sensor_msgs::msg::PointCloud2::ConstSharedPtr & points_msg_ptr, const Pose marker_pose)
{
  ...
  pcl::PointCloud<PointT>::Ptr marker_points_ptr(new pcl::PointCloud<PointT>);
  ...
  for (const auto & point : points_ptr->points) {
    ...
    if (xy_distance < param_.radius_for_extracting_marker_pointcloud) {
      ...
      ring_array.push_back(point.ring);  // PointTにringがある前提
    }
  }
