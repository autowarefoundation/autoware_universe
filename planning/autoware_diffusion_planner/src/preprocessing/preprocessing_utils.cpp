...
void normalize_input_data(InputDataMap & input_data_map, const NormalizationMap & normalization_map)
{
  ...
  for (auto & [key, value] : input_data_map) {
    // Skip normalization for ego_shape, sampled_trajectories, and *_with_z (debug only)
    if (
      key == "ego_shape" || key == "sampled_trajectories" || key == "lanes_with_z" ||
      key == "route_lanes_with_z") {
      continue;
    }
