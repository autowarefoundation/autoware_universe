...
### (2) Modify control parameter

The longitudinal PID controller's `enable_keep_stopped_until_steer_convergence` must be disabled so that the ego vehicle can resume motion under trajectories generated by the diffusion planner.
