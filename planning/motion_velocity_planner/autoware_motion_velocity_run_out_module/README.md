...
- `stop`: insert a `0` velocity ahead of the predicted collision point by the distance set in the `stop.distance_buffer` parameter.
  - to prevent the stop pose from jumping around, the previous stop pose is reused if it is before the newly calcuted stop pose, but only if the arc length between the two poses is less than the `stop.reuse_margin` parameter.
