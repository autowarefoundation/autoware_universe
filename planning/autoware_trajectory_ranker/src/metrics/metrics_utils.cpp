  ...
  const geometry_msgs::msg::Point & target, const geometry_msgs::msg::Pose & current_pose)
{
  ...
  const double denominator = 2 * transform_to_relative_coordinate2_d(target, current_pose).y;
  ...
  const geometry_msgs::msg::Point & target, const geometry_msgs::msg::Pose & current_pose)
{
  ...
  const double radius = calc_radius(target, current_pose);
  ...
  const std::shared_ptr<PredictedObjects> & objects, const size_t idx)
{
  ...
  for (const auto & object : objects->objects) {
    ...
    const double ttc = time_to_collision(ego_point, time, object);
...
double time_to_collision(const TrajectoryPoint & point1, const TrajectoryPoint & point2)
{
  ...
  if (std::abs(relative_velocity) < eps) return std::numeric_limits<double>::infinity();
...
{
  static constexpr double max_ttc_value = 10.0;
  ...
  if (object_path.size() < 2) {
    if (duration.seconds() == 0.0) {
      ...
      double ttc = time_to_collision(ego_point, object_point);
  ...
  if (dt <= 0.0) return max_ttc_value;

  const double max_time =
    dt * static_cast<double>(object_path.size() - 1);  // max time of the object path
  const double query_time = duration.seconds();
  if (query_time < 0.0) return max_ttc_value;
  ...
  if (segment_length > 1e-6) {
    ...
    const double v = segment_length / dt;  // velocity scalar in world
    ...
    obj.longitudinal_velocity_mps = 0.0;
    obj.lateral_velocity_mps = 0.0;
  }

  const double ttc = time_to_collision(ego_point, obj);
  return std::min(ttc, max_ttc_value);
}
