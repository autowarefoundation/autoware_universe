...
#ifndef MPPI__DDP__BOXQP_H_
#define MPPI__DDP__BOXQP_H_
...
public:
  ...
  explicit BoxQP(Logger* logger) : BoxQP(BoxQPOptions<Scalar>(), logger)
  {
  }
                    ...
                    const Eigen::Ref<const Vector>& x0)
  {
    ...
    for (iter = 0; iter < max_iter_; ++iter)
    {
      ...
      const auto clamped_changed =
          (clamped.template cast<int>() - old_clamped.template cast<int>()).template cast<bool>();
      if (iter == 0 || clamped_changed.any())
      {
      ...
      {
        logger_->info(
            "iter %-3d\tvalue %-9.5g\t|g| %-9.3g\treduction %-9.3g\t"
            "linesearch %g^%-2d\tn_clamped %d\n",
    ...
    {
      logger_->info(
          "Result code %d\titerations %d\tgradient %-12.6g\t"
          "final value %-12.6g\tfactorizations %d\n",
