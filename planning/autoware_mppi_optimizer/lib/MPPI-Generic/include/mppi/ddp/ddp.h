...
#ifndef MPPI__DDP__DDP_H_
#define MPPI__DDP__DDP_H_
      ...
      const Eigen::Ref<const Control>& u_max = Control::Constant(std::numeric_limits<Scalar>::infinity()))
  {
    ...
    while (current_iteration < iter_)
    {
      ...
      for (int k = H_ - 2; k >= 0; --k)
      {
        ...
        using QuuMatrix = Eigen::Matrix<Scalar, DynamicsT::ControlSize, DynamicsT::ControlSize>;
        Eigen::LDLT<QuuMatrix> ldlt(quu_);
