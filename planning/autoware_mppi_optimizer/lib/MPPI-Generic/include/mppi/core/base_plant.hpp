...
#ifndef MPPI__CORE__BASE_PLANT_HPP_
#define MPPI__CORE__BASE_PLANT_HPP_
...
protected:
  ...
  // time of the last state update that was used for optimization
  std::atomic<double> last_used_state_update_time_{ 0.0 };
...
public:
  ...
  virtual void updateState(Eigen::Ref<s_array> state, double time)
  {
    ...
    // time_since_last_opt checks if we have a new state; only publish control on new state time
  ...
  void runControlIteration(std::atomic<bool>* is_alive)
  {
    ...
    // On first iteration with state received, do not wait for timestamps to differ
    ...
    while (temp_last_used_state_update_time == temp_last_state_time && !skip_first_loop && is_alive->load())
    {
      ...
      // runControlIteration may run before state; state time may be specifically 0
    ...
    // get the most recent state and time locked together so they are synced and up to date
    ...
    else
    {
      last_optimization_stride_ =
          std::max(static_cast<int>(round(dt / this->controller_->getDt())), optimization_stride_);
    }
    // printf("calc optimization stride %f %f %f %d\n", dt,
    // temp_last_used_state_update_time, temp_last_state_time, last_optimization_stride_);
    ...
    // Compute the nominal control sequence
    controller_->computeControl(state, last_optimization_stride_);
    ...
    // TODO(autoware): make sure this is zero by default
  ...
  void runControlLoop(std::atomic<bool>* is_alive)
  {
    ...
    while (is_alive->load())
    {
      ...
      // last_used_state_update_time_, last_optimization_stride_);
      ...
      // printf("sleep: %f loop_time %f at time %f", sleep_duration_,
      // optimize_loop_duration_, getCurrentTime());
...
#endif  // MPPI__CORE__BASE_PLANT_HPP_
