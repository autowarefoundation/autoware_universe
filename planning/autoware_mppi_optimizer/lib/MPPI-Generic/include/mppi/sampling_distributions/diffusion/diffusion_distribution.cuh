...
template <class CLASS_T, template <int> class PARAMS_TEMPLATE = DiffusionParams, class DYN_PARAMS_T = DynamicsParams>
class DiffusionDistributionImpl : public GaussianDistributionImpl<CLASS_T, PARAMS_TEMPLATE, DYN_PARAMS_T>
{
public:
  ...
  __host__ void setWeightsPath(const std::string& path)
  {
    pending_weights_path_ = path;
  }
  ...
  __host__ void setObstacleContextFromMovingCars(float ego_x, float ego_y, float ego_yaw, float ego_vel,
                                                 float boundary_left, float boundary_right,
                                                 const std::vector<mppi::cost::MovingCarObstacle>& cars, float sim_time);

  __host__ void setObstacleContextFromParkedCars(float ego_x, float ego_y, float ego_yaw, float ego_vel,
                                                 float boundary_left, float boundary_right,
...
class DiffusionDistribution
  : public DiffusionDistributionImpl<DiffusionDistribution<DYN_PARAMS_T>, DiffusionParams, DYN_PARAMS_T>
{
public:
  using PARENT_CLASS = DiffusionDistributionImpl<DiffusionDistribution, DiffusionParams, DYN_PARAMS_T>;
  using SAMPLING_PARAMS_T = typename PARENT_CLASS::SAMPLING_PARAMS_T;

  DiffusionDistribution(cudaStream_t stream = 0) : PARENT_CLASS(stream)
  {
  }
  DiffusionDistribution(const SAMPLING_PARAMS_T& params, cudaStream_t stream = 0) : PARENT_CLASS(params, stream)
  {
  }
};
