 ...
 *   Per obstacle slot (up to 4): dx, dy, cos(d_yaw), sin(d_yaw), half_length, half_width, vx_ego, vy_ego
 *   Scene tail: boundary_left, boundary_right, ego_vel
 *
 * Positions/velocities are in the ego rear-axle frame at MPPI decision time. Unused obstacle slots are zero.
 */
#pragma once

#ifndef MPPI_SAMPLING_DIFFUSION_OBSTACLE_CONTEXT_HPP_
#define MPPI_SAMPLING_DIFFUSION_OBSTACLE_CONTEXT_HPP_
...
inline void encodeObstacleSlot(const float ego_x, const float ego_y, const float ego_cos, const float ego_sin,
                               const float obs_x, const float obs_y, const float obs_yaw, const float obs_half_length,
                               const float obs_half_width, const float obs_vx_world, const float obs_vy_world,
                               float* slot_out)
{
...
inline void encodeDiffusionObstacleContext(const float ego_x, const float ego_y, const float ego_yaw, const float ego_vel,
                                         const float boundary_left, const float boundary_right, const float* obs_x,
                                         const float* obs_y, const float* obs_yaw, const float* obs_half_length,
                                         const float* obs_half_width, const float* obs_vx_world,
                                         const float* obs_vy_world, const int num_obstacles, float* out)
{
  ...
  std::sort(ranked, ranked + keep,
            [](const RankedObstacle& a, const RankedObstacle& b) { return a.dist_sq < b.dist_sq; });
  ...
  for (int slot = 0; slot < keep; ++slot)
  {
    ...
    encodeObstacleSlot(ego_x, ego_y, ego_cos, ego_sin, obs_x[i], obs_y[i], obs_yaw[i], obs_half_length[i],
...
inline void gatherActiveMovingCarObstacles(const std::vector<mppi::cost::MovingCarObstacle>& cars,
                                           const float sim_time, std::vector<float>& obs_x, std::vector<float>& obs_y,
                                           std::vector<float>& obs_yaw, std::vector<float>& obs_half_length,
                                           std::vector<float>& obs_half_width, std::vector<float>& obs_vx,
...
inline void gatherParkedCarObstacles(const std::vector<mppi::cost::ParkedCarObstacle>& cars,
    ...
    const float boundary_right, const std::vector<mppi::cost::MovingCarObstacle>& cars, const float sim_time)
{
  ...
  std::vector<float> obs_vy;
  detail::gatherActiveMovingCarObstacles(cars, sim_time, obs_x, obs_y, obs_yaw, obs_half_length, obs_half_width,
                                         obs_vx, obs_vy);

  std::vector<float> out(static_cast<size_t>(kDiffusionContextDim), 0.0F);
  encodeDiffusionObstacleContext(ego_x, ego_y, ego_yaw, ego_vel, boundary_left, boundary_right, obs_x.data(),
                                 obs_y.data(), obs_yaw.data(), obs_half_length.data(), obs_half_width.data(),
                                 obs_vx.data(), obs_vy.data(), static_cast<int>(obs_x.size()), out.data());
  return out;
}
    ...
    float* out)
{
  ...
  encodeDiffusionObstacleContext(ego_x, ego_y, ego_yaw, ego_vel, boundary_left, boundary_right, obs_x.data(),
                                 obs_y.data(), obs_yaw.data(), obs_half_length.data(), obs_half_width.data(),
                                 obs_vx.data(), obs_vy.data(), static_cast<int>(obs_x.size()), out);
}
    ...
    const float boundary_right, const std::vector<mppi::cost::ParkedCarObstacle>& cars)
{
  ...
  std::vector<float> out(static_cast<size_t>(kDiffusionContextDim), 0.0F);
  encodeDiffusionObstacleContext(ego_x, ego_y, ego_yaw, ego_vel, boundary_left, boundary_right, obs_x.data(),
                                 obs_y.data(), obs_yaw.data(), obs_half_length.data(), obs_half_width.data(), nullptr,
                                 nullptr, static_cast<int>(obs_x.size()), out.data());
  return out;
}

inline void encodeDiffusionObstacleContextFromParkedCars(
    const float ego_x, const float ego_y, const float ego_yaw, const float ego_vel, const float boundary_left,
    const float boundary_right, const std::vector<mppi::cost::ParkedCarObstacle>& cars, float* out)
{
  std::vector<float> obs_x;
  std::vector<float> obs_y;
  std::vector<float> obs_yaw;
  ...
  encodeDiffusionObstacleContext(ego_x, ego_y, ego_yaw, ego_vel, boundary_left, boundary_right, obs_x.data(),
                                 obs_y.data(), obs_yaw.data(), obs_half_length.data(), obs_half_width.data(), nullptr,
                                 nullptr, static_cast<int>(obs_x.size()), out);
}
...
#endif  // MPPI_SAMPLING_DIFFUSION_OBSTACLE_CONTEXT_HPP_
