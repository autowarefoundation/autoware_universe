...
#ifndef MPPI_SAMPLING_DIFFUSION_MODEL_HPP_
#define MPPI_SAMPLING_DIFFUSION_MODEL_HPP_
...
inline void sinusoidalTimeEmbedding(const int t, const int train_timesteps, float* out, const int dim)
{
...
inline void linearReluMatVec(const std::vector<float>& W, const std::vector<float>& b, const float* x, float* y,
...
inline void linearMatVec(const std::vector<float>& W, const std::vector<float>& b, const float* x, float* y,
                         const int out_dim, const int in_dim)
{
  for (int o = 0; o < out_dim; ++o)
  {
...
inline void diffusionPredictEps(const DiffusionModelWeights& w, const float* x_t, const float* u_nom,
                                const float* context, const int t, float* eps_out)
{
...
inline void diffusionSampleTrajectory(const DiffusionModelWeights& w, const float* u_nom, const float* context,
                                      float* u_out, std::mt19937& rng)
{
  const int traj = w.traj_dim;
  ...
  {
    const int t_val = static_cast<int>(std::lround(static_cast<float>(w.train_timesteps - 1) *
    ...
    for (int i = 0; i < traj; ++i)
    {
      x[static_cast<size_t>(i)] = inv_sqrt_alpha * (x[static_cast<size_t>(i)] - coef2 * eps[static_cast<size_t>(i)]);
    }
    if (step + 1 < infer)
    {
...
#endif  // MPPI_SAMPLING_DIFFUSION_MODEL_HPP_
