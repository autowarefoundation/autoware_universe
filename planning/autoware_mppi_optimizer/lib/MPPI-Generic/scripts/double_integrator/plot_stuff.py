...
track_radius_outer = 2 + 0.125
track_radius_inner = 2 - 0.125
...
def plot_nominal_trajectory(trajectory, nominal_trajectory, title="", savefig=0):
    ...
    for i in range(time_horizon - 1):
        axis.plot(
            nominal_trajectory[i, :, 0], nominal_trajectory[i, :, 1], "b", linewidth=1.0, alpha=0.7
        )

    axis.plot(
        nominal_trajectory[time_horizon - 1, :, 0],
        nominal_trajectory[time_horizon - 1, :, 1],
        "b",
        linewidth=1.0,
        alpha=0.7,
        label="Nom",
    ...
    axis.plot(
        trajectory[:, 0], trajectory[:, 1], "limegreen", alpha=0.95, linewidth=1.5, label="Act"
    )
    axis.plot(x_track_inner, y_track_inner, "k", linewidth=2)
    axis.plot(x_track_outer, y_track_outer, "k", linewidth=2)
    # plt.legend(bbox_to_anchor=(1.05, 1),loc='upper left')
...
def main(args):
    ...
    vanilla_robust_state_trajectory = np.load(
        data_dir + "vanilla_large_robust_state_trajectory.npy"
    )
    ...
    robust_ccm_nominal_state_used = np.load(
        data_dir + "robust_large_nominal_state_used_CCM_t_2999.npy"
    )
    robust_ccm_rfe_bound = np.load(
        data_dir + "robust_large_actual_free_energy_bound_CCM_t_2999.npy"
    )
    robust_ccm_nfe_bound = np.load(
        data_dir + "robust_large_nominal_free_energy_bound_CCM_t_2999.npy"
    )
    robust_ccm_rfe_growth_bound = np.load(
        data_dir + "robust_large_actual_free_energy_growth_bound_CCM_t_2999.npy"
    )
    ...
    robust_ccm_nfc_growth = np.load(
        data_dir + "robust_large_nominal_free_energy_growth_CCM_t_2999.npy"
    )
    ...
    plot_fe_bounded(
        robust_rc_rfc_growth, robust_rc_rfe_growth_bound, "Robust Robust Real Free Energy Growth"
    )
    ...
    plot_fe_bounded(
        robust_ccm_rfc_growth,
        (robust_ccm_rfe_growth_bound - incorrect_bound + correct_bound)[1:],
        "Robust CCM Real Free Energy Growth",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--show_fig",
        default=False,
        help="Show the figure upon completion of plotting.",
        required=False,
    )
    parser.add_argument(
        "--save_fig",
        default=False,
        help="Save the figures after they are generated.",
        required=False,
    )
