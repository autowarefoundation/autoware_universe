...
ax.set_ylabel("Log(Free Energy)", labelpad=-30, position=(0.5, 0.6))
...
(ln1,) = ax.plot(
    np.linspace(0, 1000, 100), 1 * np.ones(100), "k", alpha=0.7, linewidth=3, label="Crash"
)
...
def main(args):
    ...
    ani = FuncAnimation(
        fig,
        update,
        frames=np.arange(0, time.shape[0], 1),
        init_func=init,
        blit=False,
        interval=1000 / fps,
        repeat=False,
