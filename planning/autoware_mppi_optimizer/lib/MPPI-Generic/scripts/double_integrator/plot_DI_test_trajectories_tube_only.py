...
def callback(event):
    ...
    if event.key == "right":
        ...
        if not (
            actual[cur_timestep, 0, :] == 0
        ).all():  # Means that the system stopped saving trajectories because of task failure
            cur_timestep += 1
    elif event.key == "left":
        if cur_timestep > 0:
            cur_timestep -= 1
    ...
    axs.plot(
        actual[cur_timestep, :, 0],
        actual[cur_timestep, :, 1],
        "b",
        linewidth=2.0,
        label="actual trajectory",
    )
    axs.plot(
        actual[min_index:cur_timestep, 0, 0],
        actual[min_index:cur_timestep, 0, 1],
        "bx",
        label="actual state",
    )
    ...
    axs.plot(
        feedback[cur_timestep, :, 0],
        feedback[cur_timestep, :, 1],
        "c",
        linewidth=2.0,
        label="actual feedback trajectory",
    )
    ...
    # plt nominal state
    axs.plot(
        nominal[cur_timestep, :, 0],
        nominal[cur_timestep, :, 1],
        "g",
        linewidth=2.0,
        label="nominal trajectory",
    )
    axs.plot(
        nominal[min_index:cur_timestep, 0, 0],
        nominal[min_index:cur_timestep, 0, 1],
        "g+",
        label="nominal state",
    )

    # fix legend
    plt.legend(bbox_to_anchor=(0.80, 1.15), loc="upper left", borderaxespad=0.0)
