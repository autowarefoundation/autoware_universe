...
size_t findTrackingStartIndex(const Trajectory & trajectory, const Odometry & odometry)
{
  ...
  for (size_t i = 0; i < trajectory.points.size(); ++i) {
    const float dx = static_cast<float>(trajectory.points[i].pose.position.x) - ego_x;
    const float dy = static_cast<float>(trajectory.points[i].pose.position.y) - ego_y;
  ...
  const Trajectory & trajectory, const size_t start_idx, const mppi::path::Path2D & path)
{
  ...
  for (int k = 0; k < kRefHorizon; ++k) {
    const size_t idx = std::min(start_idx + static_cast<size_t>(k), trajectory.points.size() - 1U);
    const auto & point = trajectory.points[idx];

    mppi::path::PathReferenceSample & sample = ref[static_cast<size_t>(k)];
  ...
  const Mppi::state_trajectory & state_trajectory, std::vector<std::pair<float, float>> & points)
{
  ...
  for (int i = 0; i < horizon; ++i) {
    points.emplace_back(state_trajectory(pos_x_idx, i), state_trajectory(pos_y_idx, i));
  }
}
  ...
  std::vector<float> & host_controls)
{
  ...
  host_controls.assign(
    static_cast<size_t>(k) * static_cast<size_t>(horizon) * static_cast<size_t>(DYN::CONTROL_DIM),
    0.0F);
  ...
  const int top_n, std::vector<int> & rollout_indices, std::vector<float> & selected_costs)
{
  ...
  selected_costs.resize(static_cast<size_t>(num_rollouts));
  std::iota(rollout_indices.begin(), rollout_indices.end(), 0);
  for (int i = 0; i < num_rollouts; ++i) {
    selected_costs[static_cast<size_t>(i)] = raw_costs[static_cast<size_t>(i)];
  }
  ...
  for (int i = 0; i < top_n; ++i) {
    selected_costs[static_cast<size_t>(i)] = raw_costs[static_cast<size_t>(rollout_indices[i])];
  }
}
  ...
  FirstOrderDubinsMppiDebug & debug)
{
  ...
  for (size_t out_idx = 0; out_idx < rollout_indices.size(); ++out_idx) {
    ...
    rollout.cost = selected_costs[out_idx];
...
struct FirstOrderDubinsMppiInterface::Impl
{
  ...
  void setup()
  {
    ...
    cost_params.boundary_threshold = 1.5;
    cost_params.boundary_threshold_left = -1.0F;
    cost_params.boundary_threshold_right = -1.0F;
    cost_params.obstacle_collision_margin = 0.5F;
    ...
    RCLCPP_INFO(
      ...
      "wheel_base=%.2f, max_steer=%.2f, steer_std=%.3f, ego=%.2fx%.2f, axle_to_center=%.2f, "
      "obs_margin=%.2f)",
      kMppiHorizon, kNumRollouts, kDt, kLambda, vehicle_params.wheel_base,
      vehicle_params.max_steer_angle, steer_std, vehicle_params.ego_length,
      vehicle_params.ego_width, vehicle_params.ego_axle_to_box_center,
      cost_params.obstacle_collision_margin);
  }
  ...
  const Trajectory & input, const Odometry & odometry, const TrackedObjects & tracked_objects)
{
  ...
  for (size_t i = 0; i < num_points; ++i) {
    const float tracked_x = state_trajectory(pos_x_idx, static_cast<int>(i));
    const float tracked_y = state_trajectory(pos_y_idx, static_cast<int>(i));
    const float tracked_yaw = state_trajectory(yaw_idx, static_cast<int>(i));
    const float tracked_v = state_trajectory(vel_x_idx, static_cast<int>(i));

    const float ref_x = static_cast<float>(input.points[i].pose.position.x);
    const float ref_y = static_cast<float>(input.points[i].pose.position.y);
    const float ref_v = input.points[i].longitudinal_velocity_mps;
    ...
    output.points[i].pose.position.x = tracked_x;
    output.points[i].pose.position.y = tracked_y;
    output.points[i].pose.position.z = input.points[i].pose.position.z;
    output.points[i].pose.orientation = quaternionFromYaw(tracked_yaw);
    output.points[i].longitudinal_velocity_mps = tracked_v;
  }
  ...
  RCLCPP_INFO(
    mppiLogger(),
    "MPPI tracked diffusion ref in %.1f ms: start_idx=%zu steps=%d points=%zu rollouts=%zu "
    "obstacles=%zu u_accel=%.3f u_steer=%.3f baseline_cost=%.2f max_pos_err=%.3f m "
    "max_vel_err=%.3f m/s",
    elapsed_ms, impl_->tracking_start_idx, impl_->step_count, num_points,
