...
def plot_test(test_name, input_csv, output_csv, title=None):
    ...
    ax.annotate(f'Max: {max_vel:.1f} km/h\nMin: {min_vel:.1f} km/h',
...
def plot_all_tests(export_dir, output_dir=None):
    ...
    for test_name, input_file, output_file in tests:
        fig = plot_test(
            test_name,
            os.path.join(export_dir, input_file),
            os.path.join(export_dir, output_file)
        )
...
def main():
    ...
    parser.add_argument('--dir', '-d', default='/tmp/continuous_jerk_smoother',
