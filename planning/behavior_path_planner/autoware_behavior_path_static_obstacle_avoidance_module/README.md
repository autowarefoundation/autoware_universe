...
| Is the object separated from the path by an uncrossable boundary?                              | Pedestrian, Bicycle, Unknown | The module doesn't avoid objects behind an uncrossable boundary because they cannot enter the ego lane without crossing a physical boundary. An object overlapping the boundary, or whose predicted path goes around it or through one of its gaps, is still avoided. The boundary linestring types are given by `target_filtering.uncrossable_linestring_types` (default `["guard_rail"]`, an empty list disables the check). | Never avoid it.                 |
