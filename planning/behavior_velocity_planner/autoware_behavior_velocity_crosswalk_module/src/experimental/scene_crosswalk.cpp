  ...
  const PlannerData & planner_data) const
{
  ...
  for (double s = crosswalk_attention_range.first; s <= crosswalk_attention_range.second; s += sample_interval) {
    ...
    if (target_s_front >= 0.0 && target_s_front <= sparse_resample_path.length() &&
