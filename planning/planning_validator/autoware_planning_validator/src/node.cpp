...
void PlanningValidatorNode::validate(const std::shared_ptr<const PlanningValidatorData> & data)
{
  ...
  s->is_valid_trajectory_shift =
    data->last_valid_trajectory
      ? checkTrajectoryShift(
          trajectory, *data->last_valid_trajectory, data_->current_kinematics->pose.pose)
      : true;

  const auto & resampled_trajectory = *data->resampled_current_trajectory;

  s->is_valid_relative_angle = checkValidRelativeAngle(resampled_trajectory);
  s->is_valid_curvature = checkValidCurvature(resampled_trajectory);
  s->is_valid_lateral_acc = checkValidLateralAcceleration(resampled_trajectory);
  s->is_valid_lateral_jerk = checkValidLateralJerk(resampled_trajectory);
  s->is_valid_steering = checkValidSteering(resampled_trajectory);
  s->is_valid_steering_rate = checkValidSteeringRate(resampled_trajectory);
...
bool PlanningValidatorNode::checkValidSize(const Trajectory & trajectory)
{
  validation_status_->trajectory_size = trajectory.points.size();
  return trajectory.points.size() >= 2;
}

bool PlanningValidatorNode::checkValidFiniteValue(const Trajectory & trajectory)
{
...
bool PlanningValidatorNode::checkValidInterval(const Trajectory & trajectory)
{
...
bool PlanningValidatorNode::checkValidRelativeAngle(const Trajectory & trajectory)
{
...
bool PlanningValidatorNode::checkValidCurvature(const Trajectory & trajectory)
{
...
bool PlanningValidatorNode::checkValidLateralAcceleration(const Trajectory & trajectory)
{
...
bool PlanningValidatorNode::checkValidLateralJerk(const Trajectory & trajectory)
{
...
bool PlanningValidatorNode::checkValidMinLongitudinalAcceleration(const Trajectory & trajectory)
{
  if (!params_.validation_params.acceleration.enable) {
    return true;
  }
...
bool PlanningValidatorNode::checkValidMaxLongitudinalAcceleration(const Trajectory & trajectory)
{
  if (!params_.validation_params.acceleration.enable) {
    return true;
  }
...
bool PlanningValidatorNode::checkValidSteering(const Trajectory & trajectory)
{
  ...
  const auto [max_steering, i] =
    calcMaxSteeringAngles(trajectory, data_->vehicle_info.wheel_base_m);
...
bool PlanningValidatorNode::checkValidSteeringRate(const Trajectory & trajectory)
{
  if (!params_.validation_params.steering.enable) {
    return true;
  }
  ...
  const auto [max_steering_rate, i] =
    calcMaxSteeringRates(trajectory, data_->vehicle_info.wheel_base_m);
...
bool PlanningValidatorNode::checkValidVelocityDeviation(const Trajectory & trajectory)
{
  ...
  const auto idx_opt = data_->get_current_nearest_point_index();
  ...
  validation_status_->velocity_deviation = std::abs(
    ...
    data_->current_kinematics->twist.twist.linear.x);
...
bool PlanningValidatorNode::checkValidDistanceDeviation(const Trajectory & trajectory)
{
  if (!params_.validation_params.deviation.enable) {
    return true;
  }
  ...
  validation_status_->distance_deviation = autoware_utils::calc_distance2d(
    trajectory.points.at(idx), data_->current_kinematics->pose.pose);
...
bool PlanningValidatorNode::checkValidLongitudinalDistanceDeviation(const Trajectory & trajectory)
{
  if (!params_.validation_params.deviation.enable) {
    return true;
  }
  ...
  const auto ego_pose = data_->current_kinematics->pose.pose;
...
bool PlanningValidatorNode::checkValidYawDeviation(const Trajectory & trajectory)
{
  if (!params_.validation_params.deviation.enable) {
    return true;
  }
  ...
  const auto interpolated_trajectory_point =
    motion_utils::calcInterpolatedPoint(trajectory, data_->current_kinematics->pose.pose);
  validation_status_->yaw_deviation = std::abs(angles::shortest_angular_distance(
    ...
    tf2::getYaw(data_->current_kinematics->pose.pose.orientation)));
...
bool PlanningValidatorNode::checkValidForwardTrajectoryLength(const Trajectory & trajectory)
{
  if (!params_.validation_params.forward_trajectory_length.enable) {
    return true;
  }

  const auto ego_speed = std::abs(data_->current_kinematics->twist.twist.linear.x);
  if (ego_speed < 1.0 / 3.6) {
    return true;  // Ego is almost stopped.
  }

  const auto forward_length = autoware::motion_utils::calcSignedArcLength(
    trajectory.points, data_->current_kinematics->pose.pose.position, trajectory.points.size() - 1);
...
bool PlanningValidatorNode::checkTrajectoryShift(
